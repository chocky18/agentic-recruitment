...
const ChatBot = () => {
    ...
    const wsUrl = "ws://127.0.0.1:8000/ws"; // WebSocket URL
    ...
    const uploadFile = async () => {
        ...
        try {
            const res = await fetch("http://127.0.0.1:8000/upload_resumes", {
                method: "POST",
                body: formData,
            });
            ...
            if (res.ok && result.uploaded_files && result.uploaded_files.length > 0) {
                setMessages((prev) => [
                    ...prev,
                    {
                        from: "agent",
